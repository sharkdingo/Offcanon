...
9. In the MySQL profile, lifecycle events, evidence and promotion journals survive process restart. Startup immediately audits every open journal. Active leases are reported and keep the project blocked; expired work is claimed under the project lock. `APPLYING` plus candidate fingerprint can restore `PROMOTED`, base fingerprint restores `VERIFIED`, and ambiguous state becomes `RECOVERY_REQUIRED`.
...
The hand-written agent loop enforces a step limit, overall deadline, bounded transient-model retries, sequential tool execution and repeated-failure termination. Durable context events include a base snapshot ID and context hash. On a later experiment in the same session, PICO carries forward only intent and summary; it labels the summary as stale reasoning that must be checked again.
...
The local workspace remains application-level isolation rather than an OS security sandbox. Shell policy rejects absolute paths, parent traversal, environment expansion, shell metacharacters, nested interpreters and network utilities; a deployment that needs hostile-code guarantees must add a container or OS sandbox around the worker. MySQL/Redis provide durable storage and renewable advisory coordination, but a multi-instance deployment still needs a shared data root. An active journal lease is detected immediately after restart but is not force-claimed until expiry, because ownership loss cannot otherwise be proven safely.
