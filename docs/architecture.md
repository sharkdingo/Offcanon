...
15. Changing a project's acceptance commands is serialized under the project lock. Active runs, verification and promotion still block the edit; an already-passed but unapplied result is moved back to sealed `AGENT_COMPLETED` with its snapshot/evidence intact, then can be re-verified under the new policy. Verified task-memory facts sourced from that result are retained as immutable history but are marked stale in later projections until the result passes again.
...
Runtime directories are disposable materializations, not a second history store. Active and recovery experiments, open or unresolved promotion journals, and every snapshot referenced by a lifecycle row remain protected so historical diff, promotion and continuation stay reproducible. Unreferenced snapshot materializations, terminal mutable workspaces, verification attempts and unprotected promotion candidates follow the configured retention windows. Lifecycle rows, verification evidence and snapshots are not deleted by runtime cleanup; `run_events` are a bounded activity stream (currently at most 2,000 events per experiment), so older activity can be removed while the authoritative result and evidence remain. A failed, stale or cancelled run without a sealed result keeps its partial workspace until a durable successor has forked it; after that eviction, the diff endpoint reports that the change set is unavailable rather than presenting an empty diff.

## Run activity contract

The workspace and review surfaces consume the durable `run_events` stream over SSE. The stream exposes public, auditable progress rather than hidden chain-of-thought: `MODEL_REQUEST`/`MODEL_RESPONSE` describe a model turn, `AGENT_PROGRESS` carries a short user-facing action note, `TOOL_CALL` records a display-safe tool name and bounded metadata, and the matching `TOOL_RESULT` records completion or failure. A dispatched tool call always gets a terminal result event, including cancellation, timeout and indeterminate execution (`success=false`, with an error code); the result is written before context fitting can fail. Context snapshots/compaction, verification, memory and promotion events explain retention and state changes without exposing full prompts or tool arguments. The UI keeps a recent in-memory window and labels gaps caused by the event retention policy; the review panel offers the retained payloads for audit.

## Experiment state contract

The durable `ExperimentStatus` enum has 15 values. `AGENT_COMPLETED` is intentionally used for two adjacent boundaries and is disambiguated by `resultSnapshotId`: a null value means the agent summary was saved but result sealing is still in progress; a non-null value means the immutable result is sealed. When that sealed result has not been verified yet (because the project has no commands or verification was interrupted), the UI presents the user-facing state as **Awaiting verification** or **Ready to continue** without adding a second enum value.

| State | Meaning | User actions |
| --- | --- | --- |
| `CREATED` | Intent row exists; isolation has not started. | Wait for preparation or recover/continue after an interrupted startup. |
| `SNAPSHOTTING` | Canonical base and isolated workspace are being captured. | Wait; the canonical project is unchanged. |
| `READY_TO_RUN` | The isolated workspace is ready. | Start (after the model gate), or cancel. |
| `RUNNING` | Agent model and tools are executing. | Observe live activity or cancel. |
| `AGENT_COMPLETED` (unsealed) | Agent summary is durable; result snapshot is not yet attached. | Wait for sealing; the row remains protected from policy edits. |
| `AGENT_COMPLETED` (sealed) | Immutable result is retained but trusted verification has not completed, or a policy change invalidated the earlier pass. | Run/re-run verification when commands exist; otherwise edit commands or continue into a successor. |
| `VERIFYING` | Trusted commands are running against the sealed result. | Observe or cancel; interruption returns to the sealed waiting boundary. |
| `VERIFIED` | Trusted verification passed; result awaits review. | Inspect evidence/diff, apply to canonical, or continue with a successor. |
| `REJECTED` | Verification or promotion checks failed; canonical was not overwritten. | Inspect the failure, continue fixing, or re-run verification when the sealed result remains available. |
| `STALE` | Canonical changed relative to the experiment base. | Acknowledge and continue from a fresh canonical snapshot. |
| `PREPARING_PROMOTION` | An application candidate is being prepared and checked. | Wait; project settings and competing runs are locked. |
| `PROMOTING` | Guarded writes to canonical are in progress. | Wait; recovery journaling protects against partial application. |
| `PROMOTED` | The verified result was written to canonical. | Inspect the receipt or continue with a new successor. |
| `RECOVERY_REQUIRED` | A promotion journal is ambiguous or unresolved. | Reconcile the recorded canonical state before any new application. |
| `FAILED` | The run stopped without a usable successful lifecycle outcome. | Inspect activity; retry transient/limit failures or continue the intent. |
| `CANCELLED` | The user or runtime stopped the run. | Inspect the retained record and continue from a fresh successor. |

The normal forward path is `CREATED -> SNAPSHOTTING -> READY_TO_RUN -> RUNNING -> AGENT_COMPLETED(sealed) -> VERIFYING -> VERIFIED -> PREPARING_PROMOTION -> PROMOTING -> PROMOTED`. Verification may instead end at `REJECTED`, and canonical fingerprint changes may end at `STALE`. Failures and cancellation are terminal records; a sealed verification interruption returns to `AGENT_COMPLETED(sealed)` so the same immutable snapshot can be checked again. Continuation always creates a successor experiment and never revives or mutates the source record.
...
POST /api/projects/new    -> create an empty local directory, initialize Git, and register it
POST /api/experiments/{id}/verify -> re-run trusted verification for a sealed result
```
...
The new-project flow is intentionally narrow: it accepts an absolute path on the backend machine, requires an existing writable parent, creates only the final directory (or initializes an already-empty directory), runs a parameterized `git init`, and then applies the same canonical-root and data-directory overlap checks as an opened repository. It never initializes a non-empty directory, follows symbolic-link components, or creates missing parent directories. A project may start with zero to twenty acceptance commands (each at most 1,000 characters); a sealed result remains awaiting verification until commands are configured, after which the sealed result can be re-verified explicitly. The name is a 200-character display label (usually suggested from the selected directory), while the path is capped at 4,096 characters. The resulting canonical path remains immutable after registration.
