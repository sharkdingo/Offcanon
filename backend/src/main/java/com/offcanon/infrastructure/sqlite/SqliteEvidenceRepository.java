...
public class SqliteEvidenceRepository implements EvidenceRepository {
    ...
    public SqliteEvidenceRepository(JdbcTemplate jdbc) {
        this.jdbc = jdbc;
    }
    ...
    public Evidence save(Evidence item) {
        try {
            jdbc.update(INSERT, item.id().toString(), item.experimentId().toString(), item.snapshotId().toString(), item.kind(),
                    item.command(), item.cwd(), item.exitCode(), item.stdout(), item.stderr(), SqliteValues.epochMicros(item.startedAt()),
                    SqliteValues.epochMicros(item.completedAt()), item.duration().toMillis(), item.timedOut(), item.trusted(),
    ...
    private Evidence map(ResultSet rs, int row) throws SQLException {
        return new Evidence(UUID.fromString(rs.getString("id")), UUID.fromString(rs.getString("experiment_id")),
                ...
                rs.getInt("exit_code"), rs.getString("stdout"), rs.getString("stderr"), SqliteValues.instant(rs, "started_at"),
                SqliteValues.instant(rs, "completed_at"), Duration.ofMillis(rs.getLong("duration_millis")),
