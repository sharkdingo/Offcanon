...
public class SqliteSnapshotRepository implements SnapshotRepository {
    ...
    public SqliteSnapshotRepository(JdbcTemplate jdbc, ObjectMapper mapper) {
        this.jdbc = jdbc;
        this.mapper = mapper;
    }
    ...
    public Snapshot save(Snapshot snapshot) {
        try {
            jdbc.update(INSERT, snapshot.id().toString(), snapshot.projectId().toString(), snapshot.fingerprint(),
                    snapshot.materializedPath().toString(), SqliteValues.epochMicros(snapshot.capturedAt()),
    ...
    private Snapshot map(ResultSet rs, int row) throws SQLException {
        return new Snapshot(UUID.fromString(rs.getString("id")), UUID.fromString(rs.getString("project_id")),
                ...
                SqliteValues.instant(rs, "captured_at"), jsonList(rs.getString("included_files")), jsonExcluded(rs.getString("excluded_files")));
    }
