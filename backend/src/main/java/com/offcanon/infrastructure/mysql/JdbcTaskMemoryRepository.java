...
@Repository
@Profile("mysql")
public class JdbcTaskMemoryRepository implements TaskMemoryRepository {
    ...
    public JdbcTaskMemoryRepository(JdbcTemplate jdbc, ObjectMapper mapper) {
        this.jdbc = jdbc;
        this.mapper = mapper;
    }
    ...
    public TaskMemoryRevision append(TaskMemoryRevision revision) {
        try {
            jdbc.update(INSERT, revision.id().toString(), revision.projectId().toString(),
                    ...
                    Timestamp.from(revision.createdAt()), revision.sequence());
            return revision;
